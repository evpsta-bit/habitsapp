...
Deliverables live in cloud services (Firebase / Firestore) where the user can access them.  
...
### 1. Inline "Pulse" Editing
- Used for renaming habits directly on the card.
- Combines optimistic UI updates (local state) with background cloud sync (Firestore).
- Visual cue: Violet pulsing bottom border.

### 2. Support Dock & Utility Cluster
- External links (Support Project, Help) are grouped in a vertical glass dock on desktop.
- On mobile, these evolve into a horizontal **Top Action Cluster** icon row for a native system feel.

### 3. Automated Deployment
- Use `./deploy.sh` to build the frontend and push to Firebase.
- Script handles environment verification and multi-resource deployment (Hosting + Firestore).
